...
Phaser.Filter.Shadow5 = function(game) {
    ...
    var header = [
        ...
        !Performance.UseFloat
            ? "float checkBitF(float val, float bit) { float f = pow(2., floor(mod(bit, 16.))); float vf = val - mod(val,f); if (abs(floor(vf/f/2.)*2.*f - (vf-f)) < 1.) return 1.; return 0.;  }"
         //1 if match, 0 if not
